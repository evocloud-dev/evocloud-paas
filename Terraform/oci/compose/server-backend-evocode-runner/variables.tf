...
variable "EVOCODE_RUNNER_OCPU" {
  description = "Total number of CPU cores"
  type        = number

  validation {
    condition = var.EVOCODE_RUNNER_OCPU >= 1 && var.EVOCODE_RUNNER_OCPU <= 64
    error_message = "OCPUs must be between 1 and 64"
  }
}

variable "EVOCODE_RUNNER_ORAM" {
  description = "Total number of RAM Memory in GB"
  type        = number

  validation {
    condition = var.EVOCODE_RUNNER_ORAM >= 1 && var.EVOCODE_RUNNER_ORAM <= 1024
