...
# RDP Server VM
#--------------------------------------------------
resource "oci_core_instance" "rdp_server" {
  display_name                            = var.RDP_SHORT_HOSTNAME
  compartment_id                          = var.OCI_TENANCY_ID
  ...
  source_details {
    ...
    boot_volume_vpus_per_gb = var.RDP_BASE_VOLUME_TYPE
  }
...
resource "terraform_data" "rdp_server_configuration" {
  depends_on = [oci_core_instance.rdp_server]
  ...
  provisioner "local-exec" {
    command = <<EOF
      ${var.ANSIBLE_DEBUG_FLAG ? "ANSIBLE_DEBUG=1" : ""} ANSIBLE_PIPELINING=True ansible-playbook --timeout 60 /home/${var.CLOUD_USER}/EVOCLOUD/Ansible/server-dmz-rdp.yml --forks 10 --inventory-file ${oci_core_instance.rdp_server.private_ip}, --user ${var.CLOUD_USER} --private-key ${var.NODE_PRIVATE_KEY_PAIR} --vault-password-file /home/${var.CLOUD_USER}/EVOCLOUD/Ansible/secret-vault/ansible-vault-pass.txt --ssh-common-args "-o 'StrictHostKeyChecking=no' -o 'ControlMaster=auto' -o 'ControlPersist=120s'" --extra-vars "ansible_secret=/home/${var.CLOUD_USER}/EVOCLOUD/Ansible/secret-vault/secret-store.yml server_ip=${oci_core_instance.rdp_server.private_ip} idam_server_ip=${var.idam_server_ip} idam_short_hostname=${var.IDAM_SHORT_HOSTNAME} server_short_hostname=${var.RDP_SHORT_HOSTNAME} domain_tld=${var.DOMAIN_TLD} server_timezone=${var.DEFAULT_TIMEZONE} metadata_ns_ip=${var.OCI_METADATA_NS} idam_replica_ip=${var.idam_replica_ip} cloud_platform=${var.CLOUD_PLATFORM}"
    EOF
    #Ansible logs
    environment = {
