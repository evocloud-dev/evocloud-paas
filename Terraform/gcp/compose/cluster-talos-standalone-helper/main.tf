...
resource "terraform_data" "evotalos_standalone_deployment" {
  ...
  provisioner "remote-exec" {
    inline = [
      "export GOOGLE_APPLICATION_CREDENTIALS='${var.AUTOMATION_FOLDER}/Keys/${var.GCP_JSON_CREDS}'",
      "gcloud auth activate-service-account --key-file ${var.AUTOMATION_FOLDER}/Keys/${var.GCP_JSON_CREDS}",
