...
resource "hcloud_firewall" "evorunner_firewall" {
  name = "evorunner-firewall"
  rule {
    direction = "in"
    protocol  = "tcp"
    port      = "22"
...
resource "hcloud_server" "evocode_runner_server" {
  ...
  server_type = var.EVOCODE_RUNNER_INSTANCE_SIZE         # 2 vCPU, 4GB RAM
  ...
  firewall_ids = [hcloud_firewall.evorunner_firewall.id]
