...
resource "azurerm_public_ip" "rdp_public_ip" {
  name                = "${var.RDP_SHORT_HOSTNAME}-public-ip"
...
resource "azurerm_network_interface" "rdp-server-nic" {
  name                = "${var.RDP_SHORT_HOSTNAME}-nic"
  location            = var.rg_location
  resource_group_name = var.rg_name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.rdp_public_ip.id
  }
}

resource "azurerm_network_interface_security_group_association" "ngr_assoc" {
  network_interface_id      = azurerm_network_interface.rdp-server-nic.id
...
resource "azurerm_linux_virtual_machine" "rdp_server" {
  ...
  resource_group_name = var.rg_name
  location            = var.rg_location
  size                = var.RDP_INSTANCE_SIZE
  admin_username      = var.CLOUD_USER
  network_interface_ids = [
    azurerm_network_interface.rdp-server-nic.id,
  ]
...
#--------------------------------------------------
resource "terraform_data" "redeploy_rdp" {
  input = var.rdp_revision
}

resource "terraform_data" "rdp_server_configuration" {
  depends_on = [azurerm_linux_virtual_machine.rdp_server]
  ...
  lifecycle {
    replace_triggered_by = [terraform_data.redeploy_rdp]
  }

  provisioner "local-exec" {
    command = <<EOF
      ${var.ANSIBLE_DEBUG_FLAG ? "ANSIBLE_DEBUG=1" : ""} ANSIBLE_PIPELINING=True ansible-playbook --timeout 60 \
      /home/${var.CLOUD_USER}/EVOCLOUD/Ansible/server-dmz-rdp.yml \
      --forks 10 \
      --inventory-file ${azurerm_linux_virtual_machine.rdp_server.private_ip_address}, \
      --user ${var.CLOUD_USER} \
      ...
      --extra-vars 'ansible_secret=/home/${var.CLOUD_USER}/EVOCLOUD/Ansible/secret-vault/secret-store.yml server_ip=${azurerm_linux_virtual_machine.rdp_server.private_ip_address} idam_server_ip=${var.idam_server_ip} idam_replica_ip=${var.idam_replica_ip} idam_short_hostname=${var.IDAM_SHORT_HOSTNAME} server_short_hostname=${var.RDP_SHORT_HOSTNAME} domain_tld=${var.DOMAIN_TLD} server_timezone=${var.DEFAULT_TIMEZONE}'
    EOF
    #Ansible logs
    environment = {
