...
resource "azurerm_network_interface" "evoharbor-nic" {
  name                = "${var.EVOHARBOR_SHORT_HOSTNAME}-nic"
...
resource "azurerm_network_interface_security_group_association" "ngr_assoc" {
  network_interface_id      = azurerm_network_interface.evoharbor-nic.id
  network_security_group_id = var.ssh_sgr
}

resource "azurerm_linux_virtual_machine" "evoharbor_server" {
  name                = var.EVOHARBOR_SHORT_HOSTNAME
  resource_group_name = var.rg_name
  location            = var.rg_location
  size                = var.EVOHARBOR_INSTANCE_SIZE
  admin_username      = var.CLOUD_USER
  network_interface_ids = [
    azurerm_network_interface.evoharbor-nic.id,
  ]
...
resource "terraform_data" "evoharbor_server_configuration" {
  depends_on = [azurerm_linux_virtual_machine.evoharbor_server]
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      /home/${var.CLOUD_USER}/EVOCLOUD/Ansible/server-backend-evoharbor.yml \
      --forks 10 \
      --inventory-file ${azurerm_linux_virtual_machine.evoharbor_server.private_ip_address}, \
      --user ${var.CLOUD_USER} \
      ...
      --extra-vars 'ansible_secret=/home/${var.CLOUD_USER}/EVOCLOUD/Ansible/secret-vault/secret-store.yml server_ip=${azurerm_linux_virtual_machine.evoharbor_server.private_ip_address} idam_server_ip=${var.idam_server_ip} idam_replica_ip=${var.idam_replica_ip} idam_short_hostname=${var.IDAM_SHORT_HOSTNAME} server_short_hostname=${var.EVOHARBOR_SHORT_HOSTNAME} domain_tld=${var.DOMAIN_TLD} server_timezone=${var.DEFAULT_TIMEZONE}'
    EOF
    #Ansible logs
    environment = {
